...
class Trainer:
    ...
    def training(self):
        for epoch in range(self.start_epoch, self.args.nEpochs):
            output = non_overlap_padding2(self.args, self.val_volume, self.model, self.criterion,
                                         kernel_dim=self.args.dim)
            # output = non_overlap_padding_v1(self.args, self.val_volume, self.model, self.criterion,
            #                              kernel_dim=self.args.dim)
            ...
            #self.val_volume = self.val_volume.cpu()
        print('TEST')
        #output = non_overlap_padding(self.args, self.test_volume, self.model, self.criterion, kernel_dim=self.args.dim)
    ...
    def validate_epoch(self, epoch):
        ...
        for batch_idx, input_tuple in enumerate(self.valid_data_loader):
            with torch.no_grad():
                ...
                #print('VALIDATION {} {}'.format(loss.item(), per_ch_score))
